...
async function loadSports() {
  ...
  try {
    ...
    data.forEach((match) => {
      ...
      if (allowedSports.includes(sport) && match.stream_url) {
...
function showMatches(sport, matches) {
  ...
  if (matches.length === 0) {
    matchContainer.innerHTML += `<p class="no-matches-message">No live matches currently for ${capitalize(
      sport
    )}.</p>`;
    return;
  }
  ...
  matches.forEach((match) => {
    ...
    const goalScorersHtml =
      Array.isArray(match.goal_scorers) && match.goal_scorers.length > 0
        ? `<div class="goal-scorers">
           ...
           <ul class="goal-scorers-list">
             ${match.goal_scorers
               .map(
                 (s) =>
                   `<li>${s.player_name} (${s.time}') - ${s.score_after}</li>`
               )
               .join("")}
           </ul>
         </div>`
    ...
    const watchButtonHtml = match.stream_url
      ? `<button class="watch-button" data-stream-source="${encodeURIComponent(
          match.stream_url
        )}" data-match-id="${match.match_id}">Watch</button>`
      : "";

    matchCard.innerHTML = `
      <div class="match-header">
        <h4 class="competition-name">${
          match.competition_name || "Live Match"
        }</h4>
      ...
      <p class="score">Score: ${match.home_score_current ?? "0"} - ${
      match.away_score_current ?? "0"
    }</p>
...
function playStream(streamSource, gmid) {
  ...
  const currentMatch = currentLoadedMatches.find((m) => m.match_id === gmid);
  if (currentMatch) {
    const title = document.createElement("h3");
    title.className = "playing-match-title";
    title.textContent = `Now Playing: ${currentMatch.home_team_name} vs ${currentMatch.away_team_name}`;
    videoPlayerContainer.prepend(title);
  }

  const proxiedUrl = `${BASE_URL}/proxy-stream?url=${streamSource}`;
  ...
  if (Hls.isSupported()) {
    ...
    hls.on(Hls.Events.ERROR, (event, data) => {
      if (data.fatal) {
        switch (data.type) {
          ...
          default:
            videoElement.src = "";
            videoPlayerContainer.innerHTML += '<p class="error-message">Stream error. Try another match.</p>';
            hls.destroy();
            window.hlsInstance = null;
        }
      }
    ...
    videoPlayerContainer.innerHTML += '<p class="error-message">Your browser does not support HLS.</p>';
  }
}
